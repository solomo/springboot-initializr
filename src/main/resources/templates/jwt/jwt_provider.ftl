...
import ${config.packageName}.security.AccessToken;
 ...
 * </p>
 * <p>
 * 一个完整的JwtToken由三部分组成：头部+负载信息+签名 header 存放JwtToken签名的算法 | token的类型：{"alg": "HS512","typ": "JWT"}
 * payload 主要存放用户名、创建时间、生成时间：{"sub":"wang","created":1489079981393,"exp":1489684781} signature
 * 生成算法：HMACSHA512(base64UrlEncode(header) + "." +base64UrlEncode(payload),secret)
 * <p>
 *
 * @author solom
 * @Version 1.0.0
...
public class JwtProvider {
   ...
   * 根据用户信息生成token
   */
  public AccessToken createToken(UserEntity userDetails) {
    return createToken(userDetails.getId(), userDetails.getUsername());
  }

  /**
   * 生成token 参数是我们想放入token中的字符串
   */
  public AccessToken createToken(Integer id, String subject) {
    // 当前时间
    ...
    String token = jwtProperties.getTokenPrefix() + Jwts.builder()
        ...
        .compact();
    return AccessToken.builder().loginAccount(subject).token(token).expirationTime(expirationDate)
        .build();
  }
  ...
  /**
   * 刷新token 过滤器会对请求进行验证，所以这里可以不必验证
   *
  ...
  public AccessToken refreshToken(String oldToken) {
    ...
    if (tokenRefreshJustBefore(claims)) {
      ...
      return createToken((Integer) claims.get("uid"), claims.getSubject());
    }
  }
  ...
  private boolean tokenRefreshJustBefore(Claims claims) {
    ...
    //刷新时间在创建时间的指定时间内
    if (refreshDate.after(claims.getExpiration()) && refreshDate
        .before(DateUtil.offsetSecond(claims.getExpiration(), 1800))) {
      return true;
    }
    return false;
  }
