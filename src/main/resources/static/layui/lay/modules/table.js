...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'],
    function(exports) {
        ...
        //字符常量
        , MOD_NAME = 'table', ELEM = '.layui-table', THIS = 'layui-this',
            SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled',
            NONE = 'layui-none'

        , ELEM_VIEW = 'layui-table-view', ELEM_TOOL = '.layui-table-tool',
            ELEM_BOX = '.layui-table-box', ELEM_INIT = '.layui-table-init',
            ELEM_HEADER = '.layui-table-header', ELEM_BODY = '.layui-table-body',
            ELEM_MAIN = '.layui-table-main', ELEM_FIXED = '.layui-table-fixed',
            ELEM_FIXL = '.layui-table-fixed-l',
            ELEM_FIXR = '.layui-table-fixed-r', ELEM_TOTAL = '.layui-table-total',
            ELEM_PAGE = '.layui-table-page', ELEM_SORT = '.layui-table-sort',
        ...
        , TPL_HEADER = function(options) {
            ...
            return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" ',
                '{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>', '<thead>', '{{# layui.each(d.data.cols, function(i1, item1){ }}', '<tr>', '{{# layui.each(item1, function(i2, item2){ }}', '{{# if(item2.fixed && item2.fixed !== "right"){ left = true; } }}', '{{# if(item2.fixed === "right"){ right = true; } }}',
                function() {
                ...
                rowCols +
                ' {{# if(item2.unresize || item2.colGroup){ }}data-unresize="true"{{# } }} class="{{# if(item2.hide){ }}layui-hide{{# } }}{{# if(isSort){ }} layui-unselect{{# } }}{{# if(!item2.field){ }} layui-table-col-special{{# } }}">', '<div class="layui-table-cell laytable-cell-', '{{# if(item2.colGroup){ }}', 'group', '{{# } else { }}', '{{d.index}}-{{i1}}-{{i2}}', '{{# if(item2.type !== "normal"){ }}', ' laytable-cell-{{ item2.type }}', '{{# } }}', '{{# } }}', '" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>', '{{# if(item2.type === "checkbox"){ }}' //复选框
                ,
                '<input type="checkbox" name="layTableCheckbox" lay-skin="primary" lay-filter="layTableAllChoose" {{# if(item2[d.data.checkName]){ }}checked{{# }; }}>', '{{# } else { }}', '<span>{{item2.title||""}}</span>', '{{# if(isSort){ }}',
                '<span class="layui-table-sort layui-inline"><i class="layui-edge layui-table-sort-asc" title="升序"></i><i class="layui-edge layui-table-sort-desc" title="降序"></i></span>', '{{# } }}', '{{# } }}', '</div>', '</th>', (options.fixed ? '{{# }; }}' : ''), '{{# }); }}', '</tr>', '{{# }); }}', '</thead>', '</table>'
            ].join('');
        }
        ...
        TPL_BODY = ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" ',
            '{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>', '<tbody></tbody>', '</table>'
        ].join('')
        ...
        TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
            ...
            '<tbody><tr><td><div class="layui-table-cell" style="visibility: hidden;">Total</div></td></tr></tbody>', '</table>', '</div>', '{{# } }}'
        ...
        Class.prototype.render = function() {
            ...
            if (options.cols.length > 1) {
                ...
                th.height(
                    that.layHeader.height() - 1 - parseFloat(th.css('padding-top')) -
                ...
                layui.each(item1, function(i2, item2) {
                    ...
                    if (item2.colGroup || item2.colspan > 1) {
                        ...
                        layui.each(options.cols[i1 + 1], function(i22, item22) {
                            ...
                            if (item22 === undefined || item22.HAS_PARENT || (childIndex > 1 && childIndex ==
                                    item2.colspan)) {
                                return;
                            }
        ...
        Class.prototype.setParentCol = function(hide, parentKey) {
            ...
            , parentTh = that.layHeader.find(
                    'th[data-key="' + options.index + '-' + parentKey + '"]') //获取父列元素
        ...
        Class.prototype.setColsWidth = function() {
            ...
            cntrWidth = cntrWidth - function() {
                return (options.skin === 'line' || options.skin === 'nob') ? 2 :
                    colNums + 1;
            ...
            that.eachCols(function(i3, item3) {
                ...
                if (item3.width === 0) {
                    that.getCssRule(options.index + '-' + item3.key, function(item) {
                        item.style.width = Math.floor(
                            autoWidth >= minWidth ? autoWidth : minWidth) + 'px';
                    });
                }
                ...
                //给设定百分比的列分配列宽
                    ...
                    that.getCssRule(options.index + '-' + item3.key, function(item) {
                        item.style.width = Math.floor(
                            (parseFloat(item3.width) / 100) * cntrWidth) + 'px';
                    });
                }
            ...
            //填补 Math.floor 造成的数差
            var patchNums = that.layMain.width() - that.getScrollWidth(
                    that.layMain[0]) -
        ...
        Class.prototype.reload = function(options) {
            var that = this;
            if (that.config.data && that.config.data.constructor ===
                Array) {
        ...
        Class.prototype.pullData = function(curr) {
            ...
            if (options.url) { //Ajax请求
                ...
                if (options.contentType && options.contentType.indexOf(
                        "application/json") == 0) { //提交 json 格式
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        that.setColsWidth();
                        that.loading(true);
                        typeof options.done === 'function' && options.done(res, curr,
                ...
                res[response.dataName] = options.data.concat().splice(startLimit,
                    options.limit);
        ...
        Class.prototype.renderData = function(res, curr, count, sort) {
            ...
            , render = function() { //后续性能提升的重点
                ...
                layui.each(data, function(i1, item1) {
                            ...
                            function() { //追加各种属性
                                ...
                                return item3.templet ? function() {
                                    return typeof item3.templet === 'function' ?
                                        ...
                                        laytpl(
                                            $(item3.templet).html() || String(content)).render(
                                            tplData)
                                }() : content;
                            }(), '</div></td>'
                        ].join('');
                    ...
                    trs_fixed_r.push(
                        '<tr data-index="' + i1 + '">' + tds_fixed_r.join('') +
                        '</tr>');
                });
                ...
                that.renderForm();
                typeof thisCheckedRowIndex === 'number' && that.setThisRowChecked(
                    thisCheckedRowIndex);
                that.syncCheckAll();
            ...
            //显示隐藏分页栏
            that.layPage[(count == 0 || (data.length === 0 && curr == 1)) ?
            ...
            if (data.length === 0) {
                ...
                return that.layMain.append(
                    '<div class="' + NONE + '">' + options.text.none + '</div>');
            }

            that.renderTotal(data, res); //数据合计
        ...
        Class.prototype.renderTotal = function(data, res) {
            ...
            that.layTotal[options.totalRow && data.length ? 'removeClass' :
                'addClass'](HIDE);
            if (!options.totalRow) {
                return;
            }
            ...
            layui.each(data, function(i1, item1) {
                ...
                that.eachCols(function(i3, item3) {
                    ...
                    if (item3.totalRow && (!res || !res.totalRow)) {
            ...
            that.eachCols(function(i3, item3) {
                ...
                //td内容
                var td = ['<td data-field="' + field + '" data-key="' + that.index +
                    '-' + item3.key + '" ' + function() {
                        var attr = [];
                        if (item3.align) {
                            attr.push('align="' + item3.align + '"');
                        ...
                        if (item3.totalRow) {
                            ...
                            if (typeof item3.templet == 'function') {
                                //console.log(item3, totalNums, field);
                                //console.log(item3);
                                return item3.templet(totalNums)
                            } else {
                                return totalNums[field] || text
                            }
                        } else {
                            return text || '-'
                        }
        ...
        Class.prototype.getColElem = function(parent, key) {
            var that = this,
                options = that.config;
            return parent.eq(0).find(
                '.laytable-cell-' + (options.index + '-' + key) + ':eq(0)');
        };
        ...
        Class.prototype.scrollPatch = function() {
            var that = this,
                ...
                ,
                scollHeight = that.layMain.height() - that.layMain.prop(
                    'clientHeight') //横向滚动条高度
                ,
        ...
        Class.prototype.events = function() {
                ...
                switch (events) {
                                ...
                                that.eachCols(function(i, item) {
                                    if (item.field && item.type == 'normal') {
                                        lis.push('<li><input type="checkbox" name="' + item.field +
                                            ...
                                            (item.parentKey || '') + '"' +
                                            ' lay-skin="primary" ' + (item.hide ? '' : 'checked') +
                                            ' title="' + ($(item.title).length ? ($(
                                                item.title).text() || item.field) : (item.title ||
                                                item.field)) + '"' +
                                            ' lay-filter="LAY_TABLE_TOOL_COLS" ' + (item.hideable ===
                                                false ? 'disabled' : '') + '></li>');
                                    }
                                });
                                return lis.join('');
                                    ...
                                    layui.each(options.cols, function(i1, item1) {
                                        layui.each(item1, function(i2, item2) {
                                            if (i1 + '-' + i2 === key) {
                                                ...
                                                item2.hide = !checked;
                                                that.elem.find(
                                                    '*[data-key="' + options.index + '-' + key + '"]')[checked ? 'removeClass' : 'addClass'](HIDE);
                    ...
                    case 'LAYTABLE_PRINT': //打印
                        var printWin = window.open('打印窗口', '_blank'),
                            style = ['<style>', 'body{font-size: 12px; color: #666;}',
                                'table{width: 100%; border-collapse: collapse; border-spacing: 0;}',
                                'th,td{line-height: 20px; padding: 9px 15px; border: 1px solid #ccc; text-align: left; font-size: 12px; color: #666;}', 'a{color: #666; text-decoration:none;}', '*.layui-hide{display: none}', '</style>'
                            ].join(''),
            ...
            that.elem.on('click', 'input[name="layTableCheckbox"]+', function() { //替代元素的 click 事件
                ...
                layui.event.call(checkbox[0], MOD_NAME, 'checkbox(' + filter + ')',
            ...
            that.layBody.on('click', '.' + ELEM_GRID_DOWN, function() {
                ...
                that.tipsIndex = layer.tips([
                    ...
                    (elemCell.height() + 16) + 'px;' + function() {
                        if (options.size === 'sm') {
                    ...
                    time: -1,
                    anim: -1,
                    maxWidth: (device.ios || device.android) ? 300 : that.elem.width() /
                        2,
                    isOutAnim: false,
        ...
        table.init = function(filter, settings) {
            settings = settings || {};
            var that = this,
                elemTable = filter ? $('table[lay-filter="' + filter + '"]') : $(
                    ELEM + '[lay-data]'),
        ...
        table.dataParams = ['url', 'method', 'where', 'contentType', 'headers',
            'parseData', 'request', 'response', 'data', 'page' /*, 'limit', 'limits'*/
        ];
        ...
        table.reload = function(id, options) {
            ...
            if (options.data && options.data.constructor ===
                Array) {
                delete config.data;
            }
