...
public class CommonBuilderImpl extends BaseBuilder {
  ...
  public void generation(PropertiesConfig config, String projectsRoot) throws Exception {
    ...
    File file = new File(projectsRoot + config.getArtifactId() + "/src/main/java/" + packagePath, "Common.java");
    // 写入文件
    super.writeFile(file, "common/common.ftl", info);
    file = new File(projectsRoot + config.getArtifactId() + "/src/main/java/" + packagePath, "Msg.java");
    // 写入文件
